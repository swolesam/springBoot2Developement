...
import com.tomekl007.payment.infrastructure.configuration.FilterConfig;
...
@ContextConfiguration(classes = {PaymentApplication.class, SpringSecurityWebAppConfig.class, FilterConfig.class})
@WebMvcTest(
...
public class MVCControllerTest {
  ...
  /** [SELNASR]  Because the class annotation above @AutoConfigureMockMvc , this is possible to autowire MockMvc **/
  @Autowired
  ...
  public void shouldReturnAllPayments() throws Exception {
    ...
    Iterable<Payment> payments = Arrays.asList(
        new Payment("u1", "from1", "to1", 23L));

    // [SELNASR] use MockBean declared above = paymentRepository
    when(paymentRepository.findAll()).thenReturn(payments);

    // when, then
    this.mockMvc.perform(get("/mvc/all-payments"))
        .andDo(print())
